...
describe('FocusUtil', () => {
	...
	test('getFocussedItem returns an object about the focussed item', () => {
		const mockState = { type: 'mock-type', target: 'mock-target', animateScroll: false }
		expect(FocusUtil.getFocussedItem(mockState)).toEqual({
			type: 'mock-type',
			target: 'mock-target',
			animateScroll: false
		})
		expect(mockState).toEqual({ type: 'mock-type', target: 'mock-target', animateScroll: false })
	})
	...
	test('getFocussedItemAndClear returns an object about the focussed item (but also clears the state at the same time)', () => {
		...
		expect(mockState).toEqual({ type: null, target: null, animateScroll: false })
	})
