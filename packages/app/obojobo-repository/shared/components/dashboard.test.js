...
describe('Dashboard', () => {
	...
	const expectMultiSelectControlBar = (
		...
		if (isDeleteMode) {
			...
			expect(controlBar.children[2].props.children).toEqual('Download XML')
			expect(controlBar.children[3].props.children).toEqual('Download JSON')
			expect(controlBar.children[4].props.children).toEqual('Copy')
			expect(controlBar.children[5].props.children).toEqual('Delete All')
			expect(controlBar.children[6].props.className).toEqual('close-button')
	...
	test('"Download XML" button calls downloadDocument with the correct arguments', () => {
		...
		const downloadXMLButton = component.root.findAllByType(Button)[1]
		expect(downloadXMLButton.children[0].children[0]).toBe('Download XML')

		downloadXMLButton.props.onClick()
	...
	test('"Download JSON" button calls downloadDocument with the correct arguments', () => {
		...
		const downloadJSONbutton = component.root.findAllByType(Button)[2]
	...
	test('"Copy" button makes copies of modules', async () => {
		...
		const copyButton = component.root.findAllByType(Button)[3]
		...
		// order is important - since modules are sorted alphabetically, the second module will be provided first
		expect(dashboardProps.bulkCopyModules).toHaveBeenCalledWith([
			standardMyModules[1],
			standardMyModules[0]
		])
	...
	// Doing bulk permissions management properly is a pretty tall order right now - leaving this here as a reminder
	...
	test('"Deselect All" button calls functions appropriately', () => {
		...
		// cancel button will be sixth option in 'recent' and 'all' modes
	...
	test('selecting module calls functions appropriately', () => {
		...
		// the first module will be the second one in this list
		//  because the default sorting method is alphabetical
		// this is... a bit error-prone, admittedly, but it'll do for now
		const expectedDraftId = standardMyModules[1].draftId
		...
		expect(dashboardProps.selectModules).toHaveBeenCalledWith([expectedDraftId])

		dashboardProps.selectedModules = [expectedDraftId]
		dashboardProps.multiSelectMode = true
		...
		expect(dashboardProps.deselectModules).toHaveBeenCalledTimes(1)
		expect(dashboardProps.deselectModules).toHaveBeenCalledWith([expectedDraftId])
