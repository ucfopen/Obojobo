...
function Dashboard(props) {
	...
	// bulkCopyModules takes a list of the module objects themselves, not just a list of draft IDs
	const onBulkCopyClick = () => {
		const selectedModuleObjects = props.myModules.filter(module => {
			return props.selectedModules.includes(module.draftId)
		})
		props.bulkCopyModules(selectedModuleObjects)
	}
	...
	if (props.multiSelectMode && props.selectedModules.length > 0) {
		let bulkOperationButtons = (
			<React.Fragment>
				<Button
					className="multi-select secondary-button"
					onClick={() => props.selectedModules.map(draftId => downloadDocument(draftId, 'xml'))}
				>
					Download XML
				</Button>
				<Button
					className="multi-select secondary-button"
					onClick={() => props.selectedModules.map(draftId => downloadDocument(draftId, 'json'))}
				>
					Download JSON
				</Button>
				<Button className="multi-select secondary-button" onClick={onBulkCopyClick}>
					Copy
				</Button>
				{/* Doing bulk permissions management properly is a pretty tall order right now - leaving this here as a reminder
				<Button
					className="multi-select secondary-button"
					onClick={() => props.showModulePermissions(props.selectedModules)}
				>
					Share
				</Button>
				*/}
		...
		switch (props.mode) {
			case MODE_COLLECTION:
				...
				break
			case MODE_DELETED:
				bulkOperationButtons = null
				bulkDeleteOrRestoreButton = (
					<Button
						className="multi-select secondary-button dangerous-button"
