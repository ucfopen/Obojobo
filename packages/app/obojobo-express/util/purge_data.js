...
const purgeFromTableByDate = (table, purgeDate) => db.none(`
		DELETE FROM ${table}
...
const purgeDrafts = purgeDate => db.none(`
		DELETE FROM drafts
		WHERE created_at <= $[purgeDate]
...
const purgeDraftsContent = purgeDate => db.none(`
...
const purgeMediaBinaries = purgeDate => db.none(`
...
exports.purgeData = async () => {
	...
	if(purgeMethod){
		...
		const purgeDate = moment().startOf('day').subtract(demoPurgeDaysAgo, 'days')
