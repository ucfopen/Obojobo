...
describe('Nav', () => {
	...
	test('onClick redAlert sets is-red-alert and is-not-red-alert class', () => {
		const props = {
			navState: {
				redAlert: false
			}
		...
		expect(component.hasClass('is-not-red-alert'))

		component.find('.red-alert-button').simulate('click')

		expect(NavUtil.isRedAlertEnabled).toHaveBeenCalledTimes(1)
		expect(NavUtil.setRedAlert).toHaveBeenCalledTimes(1)
		expect(component.hasClass('is-red-alert'))
	})
})
