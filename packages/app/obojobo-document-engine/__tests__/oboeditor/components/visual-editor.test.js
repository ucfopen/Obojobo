...
describe('VisualEditor', () => {
	...
	test('onKeyDown handles ArrowUp if event.defaultPrevented is false and node above has one child', () => {
		...
		const editor = {
			children: [
				...
				{ type: 'mock node 2', children: [{ text: 'one' }] },
		...
		expect(focusSelection).toEqual([1, 0])
		expect(anchorSelection).toEqual([1, 0])
	})

	test('onKeyDown handles ArrowUp if event.defaultPrevented is false and node above has multiple children', () => {
		jest.spyOn(Array, 'from').mockReturnValue([])
		const spySetSelection = jest.spyOn(Transforms, 'setSelection')
	...
	test('onKeyDown handles ArrowUp if event.defaultPrevented is false and no node above', () => {
		jest.spyOn(Array, 'from').mockReturnValue([])
		const spySetSelection = jest.spyOn(Transforms, 'setSelection')

		const editor = {
			children: [{ type: 'mock node 1', children: [{ text: 'one' }] }],
			selection: {
				anchor: { path: [0], offset: 0 },
