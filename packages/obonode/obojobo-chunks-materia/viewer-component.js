...
export default class Materia extends React.Component {
	...
	onPostMessageFromMateria(event){
		console.log(event)
		if(!this.createSrc(this.props.model.modelState.src).includes(event.origin)){
	...
	createSrc(src) {
		...
		return `/materia-lti-launch?endpoint=${encodeURI(src)}`
	}
					...
					>
						...
						{isShowing ? null : (
							<div className="click-to-load">
								...
								{ms.src === null ? null : (
									<Button ariaLabel="Click to load external content">Start Materia Widget</Button>
								)}
							</div>
						)}
						{isShowing ? (
						...
						<div className="score">
							{ this.state.unverifiedScore === false ?
								'SCORE: unknown'
								: `SCORE: ${this.state.unverifiedScore}`
							}
						</div>
					</div>
				</div>
