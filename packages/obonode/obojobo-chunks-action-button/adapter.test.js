...
import ActionButtonAdapter from './adapter'
...
describe('ActionButton adapter', () => {
	...
	test('construct builds with textGroup attributes', () => {
		...
		const tg = new TextGroup(Infinity, { indent: 0, hangingIndent: false }, [
			{ text: new StyleableText('mock-tg') }
		])
		const expected = {
			textGroup: tg,
			align: 'center'
	...
	test('clone creates a copy with a textGroup', () => {
		...
		const tgA = new TextGroup(Infinity, { indent: 0, hangingIndent: false }, [
			{ text: new StyleableText('mock-tg') }
		])
		const tgB = new TextGroup(Infinity, { indent: 0, hangingIndent: false }, [
			{ text: new StyleableText('mock-tg') }
		])
