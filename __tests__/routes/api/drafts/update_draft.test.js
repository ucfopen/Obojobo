...
describe('api draft update helper', () => {
	...
	test('calls db.one with expected args', () => {
		...
		db.one.mockImplementationOnce(() => {return Promise.resolve({id: 555})})
		...
		.then(id => {
			expect(id).toBe(555)
			expect(db.one).toBeCalledWith(expect.any(String), expect.objectContaining({
				draftId: 555,
				content: 'content'
			}))
		})
	...
	test('fails as expected', () => {
		expect.assertions(1)
		let db = oboRequire('db')
		db.one.mockImplementationOnce(() => {return Promise.reject('test error')})
