...
describe('start attempt route', () => {
	...
	it('can choose to display unseen question banks and questions sequentially', () => {
		...
		expect(
			chooseUnseenQuestionsSequentially(mockAssessmentProperties, 'qb', 2).map(node => node.id)
		).toEqual(['qb1', 'qb2'])
		...
		expect(
			chooseUnseenQuestionsSequentially(mockAssessmentProperties, 'qb1', 1).map(node => node.id)
		).toEqual(['qb1.q1'])
		...
		expect(
			chooseUnseenQuestionsSequentially(mockAssessmentProperties, 'qb1', 2).map(node => node.id)
		).toEqual(['qb1.q1', 'qb1.q2'])
	...
	it('can choose to display all question banks and questions randomly', () => {
		...
		expect(
			chooseAllQuestionsRandomly(mockAssessmentProperties, 'qb', 2).map(node => node.id)
		).toEqual(['qb2', 'qb1'])
	...
	it('can choose to display unseen question banks and questions randomly', () => {
		...
		expect(
			chooseUnseenQuestionsRandomly(mockAssessmentProperties, 'qb', 2).map(node => node.id)
		).toEqual(['qb2', 'qb1'])
