...
class Node extends React.Component {
	...
	toggleType() {
		...
		levels.forEach(levelNode => {
			...
			levelContent.bulletStyle = bulletList[oldList.indexOf(levelContent.bulletStyle) % bulletList.length]
	...
	render() {
		...
		return (
			...
			<div
				className={'text-chunk obojobo-draft--chunks--list pad'}
	...
	onKeyDown(event, change) {
		...
		if (isLine && event.key === 'Tab') {
			...
			change.value.blocks.forEach(block => {
				const level = change.value.document.getClosest(block.key,
					parent => {
