...
var assessmentReviewView = function assessmentReviewView(assessment) {
	...
	return React.createElement(
		...
		attempts.map(function (attempt, index) {
			var scoreTotal = attempts[index].scores.questionScores.reduce(function (prev, curr) {
				return prev + curr.score;
			}, 0);
			var reviewScore = scoreTotal / attempts[index].scores.questionScores.length;
			return attemptReviewComponent(attempt, reviewScore, assessment, index + 1);
		})
	);
};

var attemptReviewComponent = function attemptReviewComponent(attempt, reviewScore, assessment, attemptNumber) {
	return React.createElement(
		'div',
		{ className: 'review' },
		React.createElement(
		...
		attempt.scores.questionScores.map(function (scoreObj) {
...
var Assessment = function (_React$Component) {
	...
	_createClass(Assessment, [{
		...
		value: function getCurrentStep() {
			...
			if (!AssessmentUtil.hasAttemptsRemaining(this.props.moduleData.assessmentState, this.props.model) && this.props.model.modelState.review) {
				return 'review';
			}
		...
		value: function render() {
			...
			var assessmentScore = AssessmentUtil.getAssessmentScoreForModel(this.props.moduleData.assessmentState, this.props.model);
			var ltiStatus = AssessmentUtil.getLTIStatusForModel(this.props.moduleData.assessmentState, this.props.model);
