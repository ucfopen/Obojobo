		...
		[...questionBank.immediateChildrenSet]
			.filter(id => {
				console.log(
					'TEST',
					id,
					questionBank.draftTree.getChildNodeById(id).node.type !==
...
let getQuestionHistoryArray = questionHistory => {
	return [...questionHistory.entries()].map(item => {
		return { id: item[0], timesUsed: item[1] }
	})
}
...
module.exports = function(questionBank, attemptHistory) {
	...
	switch (opts.select) {
		case 'random-all':
			return getRandomAllQuestions(questionHistory, questionBank)
		case 'random-unseen':
			return getRandomUnseenQuestions(questionHistory, questionBank)
		case 'sequential':
		default:
			return getSequentialQuestions(questionHistory, questionBank)
			break
	}
}
