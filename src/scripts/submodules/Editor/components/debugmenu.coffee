...
DebugMenu = React.createClass
	...
	componentDidMount: ->
		document.addEventListener 'keydown', ((event) ->
			...
			if event.keyCode is 192 and not @state.listeningForKeyboard
				...
				@setState { awaken:true, listeningForKeyboard: true }
			else if @state.listeningForKeyboard
				event.preventDefault()
				@state.listeningForKeyboard = false
				console.clear()

				# console.log(JSON.stringify(__s.getSelectionDescriptor()))
				# __lo.chunks.at(0).selectAll()
				# __force({})
				# console.log(JSON.stringify(__s.getSelectionDescriptor()))
				...
				switch event.keyCode
					...
					when 84 #t
						if @state.target is 'text selection'
							...
							console.log JSON.stringify(@state.selection.getSelectionDescriptor(), null, 2)
					when 72 #h
	...
	render: ->
		if not @state.awaken then return null
		...
		text = switch @state.target
			when 'selection'
				...
				o =
					...
					desc: @state.selection.getSelectionDescriptor()
				JSON.stringify(o, null, 2)
			when 'text selection'
				JSON.stringify(@state.selection.getSelectionDescriptor(), null, 2)
			when 'chunk'
				t = ''
				for chunk in @state.virtual.chunk.all
			...
			when 'module'
				JSON.stringify @state.selection.virtual.module.toJSON(), null, 2
			else
				''
