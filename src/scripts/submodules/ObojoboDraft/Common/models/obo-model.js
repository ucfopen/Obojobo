...
let DefaultAdapter = {
	construct(attrs) { return null; },
	clone(clone) { return clone; },
	toJSON(model, json) { return json; },
	toText(model) { return ''; }
};
...
class OboModel extends Backbone.Model {
	...
	prevSibling() {
		if (this.isOrphan() || this.isFirst()) { return null; }
	...
	nextSibling() {
		if (this.isOrphan() || this.isLast()) { return null; }
	...
	replaceWith(newChunk) {
		if (this.isOrphan() || (newChunk === this)) { return; }
	...
	getParentOfType(type) {
		...
		while (model !== null) {
			if (model.get('type') === type) { return model; }
			model = model.parent;
		}
