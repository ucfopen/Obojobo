...
const ORDER = [StyleType.COMMENT, StyleType.LATEX, StyleType.LINK, StyleType.QUOTE, StyleType.BOLD, StyleType.STRIKETHROUGH, StyleType.MONOSPACE, StyleType.SUPERSCRIPT, StyleType.ITALIC];
...
var getTextNodeFragmentDescriptorsAtHelper = function(stateObj, targetStartIndex, targetEndIndex) {
	if (stateObj.curNode.nodeType === 'element') {
		return Array.from(stateObj.curNode.children).map((child) =>
...
let getTextNodeFragmentDescriptorsAt = function(rootNode, startIndex, endIndex) {
	...
	if (stateObj.start !== null) { fragmentDescriptors.unshift(stateObj.start); }
	if (stateObj.end !== null) { fragmentDescriptors.push(stateObj.end); }
...
let wrap = function(styleRange, nodeFragmentDescriptor) {
	...
	if (wrappedText.length === 0) { return; }
