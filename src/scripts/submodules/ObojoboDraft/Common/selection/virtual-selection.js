...
class VirtualSelection {
	...
	fromDOMSelection(domSelection) {
		...
		if (domSelection == null) { domSelection = null; }
		...
		let startChunkIndex = DOMUtil.findParentAttr(domSelection.startContainer, 'data-component-index');
		let endChunkIndex = DOMUtil.findParentAttr(domSelection.endContainer, 'data-component-index');

		if (!startChunkIndex || !endChunkIndex) { return; }
...
Object.defineProperties(VirtualSelection.prototype, {
	"type": {
		get() {
			switch (false) {
				case !((this.start != null ? this.start.chunk : undefined) == null) && !((this.end != null ? this.end.chunk : undefined) == null): return 'none';
				case this.start.chunk.cid === this.end.chunk.cid: return 'chunkSpan';
				case !this.start.isEquivalentTo(this.end): return 'caret';
