...
let MCAssessment = React.createClass({
	...
	getResponseData() {
		...
		for (let child of Array.from(this.props.model.children.models)) {
			...
			if (__guard__(QuestionUtil.getResponse(this.props.moduleData.questionState, child), x => x.set)) {
	...
	calculateScore() {
		...
		switch (this.props.model.modelState.responseType) {
			case 'pick-all':
				if (correct.size !== responses.size) { return 0; }
				let score = 100;
				correct.forEach(function(id) { if (!responses.has(id)) { return score = 0; } });
				return score;

			default: // pick-one | pick-one-multiple-correct
				for (let id of Array.from(Array.from(correct))) {
					if (responses.has(id)) { return 100; }
				}
		...
		switch (this.props.model.modelState.responseType) {
			case 'pick-all':
				return QuestionUtil.recordResponse(mcChoiceId, {
					set: !__guard__(QuestionUtil.getResponse(this.props.moduleData.questionState, OboModel.models[mcChoiceId]), x => x.set)
				});
		...
		>
			<span className="instructions">
			{
				(function() {
					switch(responseType)
					{
						case 'pick-one':                  return <span>Pick the correct answer</span>;
						case 'pick-one-multiple-correct': return <span>Pick one of the correct answers</span>;
						case 'pick-all':                  return <span>Pick <b>all</b> of the correct answers</span>;
					}
				})()
			}
			</span>
						...
						(
							...
							<div className="result-container">
								<p className="result incorrect">Incorrect</p>
								{
									...
									?
									<span className="pick-all-instructions">You have either missed some correct answers or selected some incorrect answers</span>
									:
									null
								}
							</div>
						...
						<div className="score">
							{
								...
								null
								:
								<div className={`feedback${responseType === 'pick-all' ? ' is-pick-all-feedback' : ' is-not-pick-all-feedback'}`}>
									{
										feedbacks.map((model => {
