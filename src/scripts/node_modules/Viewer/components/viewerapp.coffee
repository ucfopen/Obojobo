...
ViewerApp = React.createClass
	getInitialState: ->
		console.log 'navstore', NavStore, AssessmentStore, AssessmentStore.getState()
		...
		if window.location.hash.indexOf('legacy') > -1
			moduleData = require 'json!../../../../../citing-sources-mla.json'
			model = Legacy.createModuleFromObo2ModuleJSON moduleData
		...
		if window.location.hash.indexOf('file') > -1
			moduleData = @props.moduleData

			#@TODO - do this elsewhere!
			module = OboModel.create(moduleData)
			NavStore.init module
			return ({
				model: module,
				navState: NavStore.getState(),
				scoreState: ScoreStore.getState(),
		...
		if window.localStorage.__lo?
			try
				moduleData = JSON.parse(window.localStorage.__lo)
				module = OboModel.create(moduleData)
				NavStore.init module
				return {
					model: module,
	...
	# walk: (model, indent = '') ->
	# 	if model.get('type').indexOf('Chunks') > -1 then return

	# 	console.log indent, model.get('type')
	# 	for child in model.children.models
	# 		@walk child, indent + '  '

	componentWillMount: ->
		NavStore.addChangeListener @onNavChange
		ScoreStore.addChangeListener @onScoreChange
		AssessmentStore.addChangeListener @onAssessmentChange

	componentWillUnmount: ->
		NavStore.removeChangeListener @onNavChange
		ScoreStore.removeChangeListener @onScoreChange
		AssessmentStore.removeChangeListener @onAssessmentChange

	onNavChange: -> @setState { navState:NavStore.getState() }
	onScoreChange: -> @setState { scoreState:ScoreStore.getState() }
	onAssessmentChange: -> @setState { assessmentState:AssessmentStore.getState() }
	...
	render: ->
		...
		console.log 'json', @state.model

		`<div className={'viewer--viewer-app' + (this.state.navState.locked ? ' is-locked-nav' : ' is-unlocked-nav') + (this.state.navState.open ? ' is-open-nav' : ' is-closed-nav') + (this.state.navState.disabled ? ' is-disabled-nav' : ' is-enabled-nav')}>
			<Nav navState={this.state.navState} />
			<JSONInput onToggle={this.onJsonInputToggle} onChange={this.update} value={JSON.stringify(this.state.model.toJSON(), null, 2)} />
			{prevEl}
			<ModuleComponent model={this.state.model} navState={this.state.navState} scoreState={this.state.scoreState} assessmentState={this.state.assessmentState} />
			{nextEl}
		</div>`
