...
ViewerApp = React.createClass
	...
	getInitialState: ->
		...
		state =
			model: OboModel.create(@props.initialState.draft)
			navState: null
			scoreState: null
			questionState: null
		...
		AssessmentStore.init @props.initialState['ObojoboDraft.Sections.Assessment:attemptHistory']
	...
	render: ->
		...
		`<div ref="container" onMouseDown={this.onMouseDown} onScroll={this.onScroll} className={'viewer--viewer-app' + (this.state.navState.locked ? ' is-locked-nav' : ' is-unlocked-nav') + (this.state.navState.open ? ' is-open-nav' : ' is-closed-nav') + (this.state.navState.disabled ? ' is-disabled-nav' : ' is-enabled-nav') + ' is-focus-state-' + this.state.focusState.viewState}>
			<header>
				<div className="pad">
