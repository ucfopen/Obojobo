	...
	render: ->
		if(@state.open)
			return `<div style={{position:'fixed',right:0,top:0,bottom:0,width:'50%',background:'gray',zIndex:9999}}>
				...
				<textarea style={{position:'absolute',left:0,top:'20px',right:0,bottom:'40px',width:'100%', fontFamily:'monospace', fontSize:'10pt',padding:0,margin:0,boxSizing:'border-box'}} value={this.state.value} onChange={this.onChange} />
				<button style={{position:'absolute',bottom:'10px',left:0,display:'block',width:'100%'}} onClick={this.onClick}>Update</button>
			</div>`
