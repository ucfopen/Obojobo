...
class NavStore extends Store {
	...
	gotoItem(navItem) {
		...
		if (this.state.navTargetId != null) {
			if (this.state.navTargetId === navItem.id) { return; }

			let navTargetModel = __guard__(NavUtil.getNavTargetModel(this.state), x => x.processTrigger('onNavExit'));
	...
	generateNav(model, indent) {
		...
		let item = ObojoboDraft.Store.getItemForType(model.get('type'));
		...
		for (let child of Array.from(model.children.models)) {
			...
			childNavItem.fullPath = navItem.fullPath.concat(childNavItem.fullPath).filter(item => item !== '');
	...
	_clearFlags() {
		return Array.from(this.state.items).map((item) =>
			(item.flags.complete = false));
	}
}
