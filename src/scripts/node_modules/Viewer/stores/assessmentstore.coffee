...
class AssessmentStore extends Store
	constructor: ->
		...
		Dispatcher.on 'assessment:startAttempt', (payload) =>
			...
			# debugger
			if(AssessmentUtil.getNumberOfAttempts(@state, model) >= model.modelState.attempts)
				return ModalUtil.show `<SimpleDialog ok title="No more attempts">{'Sorry, you have reached the limit of ' + model.modelState.attempts + ' attempts for this assessment.'}</SimpleDialog>`
