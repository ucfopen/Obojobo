...
class NavStore extends Store
	constructor: ->
		...
		Dispatcher.on
			...
			'score:set': (payload) =>
				...
				NavUtil.setFlag payload.value.id, 'correct', (payload.value.score is 100)
		, @
	...
	init: (model, startingId, startingPath) ->
		...
		if startingId?
			...
			NavUtil.goto NavUtil.getFirst(@state).id

	buildMenu: (model) ->
		@state.itemsById = {}
		@state.itemsByPath = {}
		@state.itemsByFullPath = {}
	...
	gotoItem: (navItem) ->
		...
		if @state.navTargetId?
			if @state.navTargetId is navItem.id then return
	...
	generateNav: (model, indent = '') ->
		...
		for child in model.children.models
			...
			@state.itemsByFullPath[childNavItem.fullFlatPath] = childNavItem
