Util = require './textgroup/textgrouputil'
StyleableText = require './styleabletext'
TextGroupItem = require './textgroup/textgroupitem'
...
class TextGroup
	...
	add: (text, data) ->
		...
		addChildToGroup new TextGroupItem(text, Util.createData(data, @dataTemplate)), @
	...
	addAt: (index, text, data) ->
		...
		addChildToGroup new TextGroupItem(text, Util.createData(data, @dataTemplate)), @, index
	...
	addGroup: (group, cloneDataFn = Util.defaultCloneFn) ->
		...
		for item in group.items
			...
			itemsToAdd.push new TextGroupItem(clone.text, Util.createData(clone.data, @dataTemplate))
	...
	splitText: (index, groupIndex, cloneDataFn = Util.defaultCloneFn) ->
		item = @items[index]
	...
	deleteSpan: (startIndex, startTextIndex, endIndex, endTextIndex, mergeFn = Util.defaultMergeFn) ->
