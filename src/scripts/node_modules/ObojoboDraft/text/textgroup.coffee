...
class TextGroup
	...
	addGroup: (group, cloneDataFn = Util.defaultCloneFn) ->
		@addGroupAt group, @length - 1, cloneDataFn
	...
	remove: (index) ->
		# console.log 'remove', index, @items
		removeChildFromGroup(@items[index], @)[0]
		# console.log 'after', index, @items

		# if @items.length is 0 then debugger;
	...
	toSlice: (from, to = Infinity) ->
		removeChildFromGroup @items.slice(to + 1), @
	...
	splitText: (index, offset, cloneDataFn = Util.defaultCloneFn) ->
		item = @items[index]

		newItem = createChild(item.text.split(offset), cloneDataFn(item.data), @dataTemplate)
	...
	deleteSpan: (startIndex, startTextIndex, endIndex, endTextIndex, merge = true, mergeFn = Util.defaultMergeFn) ->
		...
		console.log 'start item', @items, startItem
		startText = startItem.text
		endText   = endItem.text

		console.log 'del span', arguments
