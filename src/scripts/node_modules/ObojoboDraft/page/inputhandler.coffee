		...
		switch event.keyCode
			...
			when Keyboard.TAB
				...
				if caretEdge is 'start' or caretEdge is 'startAndEnd'
					# ...indent
					ChunkUtil.send 'indent', selection.startChunk, selection, [event.shiftKey]
					updateFn()
					return

				# Otherwise, let the chunk figure out what to do
				ChunkUtil.send 'onTab', selection.startChunk, selection, [event.shiftKey]
				updateFn()
			...
			switch event.keyCode
				...
				when 65 #a
					if selection.chunk.type isnt 'chunkSpan'
						if ChunkUtil.send 'selectAll', selection.startChunk, selection
							event.preventDefault()
							# updateSelectionFn()
							updateFn()
							return