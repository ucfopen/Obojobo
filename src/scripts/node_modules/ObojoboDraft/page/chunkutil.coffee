...
send = (fn, chunkOrChunks, selection, data = []) ->
	if not (chunkOrChunks instanceof Array)
		return chunkOrChunks.callComponentFn fn, data

	chunks = chunkOrChunks
	results = []
	for chunk in chunks
		results.push chunk.callComponentFn fn, data
...
replaceTextsWithinSelection = (selection, newChunk) ->
	selection.virtual.start.chunk.addChunkBefore newChunk
