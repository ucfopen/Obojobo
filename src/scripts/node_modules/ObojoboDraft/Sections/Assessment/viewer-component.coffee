		...
		childEl = switch @getCurrentStep()
			when 'untested'
				...
				`<div className="untested">
					...
					<p>{'#' + this.props.model.modelState.attempts}</p>
					<p>{'#' + AssessmentUtil.getNumberOfAttemptsCompletedForModel(this.props.moduleData.assessmentState, this.props.model)}</p>
				</div>`
			...
			when 'scoreSubmitted'
				...
				if scoreAction.page?
					...
					pageModel.parent = @props.model; console.log('@TODO - FIGURE OUT A BETTER WAY TO DO THIS - THIS IS NEEDED TO GET {{VARIABLES}} WORKING')
					...
					<div className="review">
						{
							questionScores.map((function(questionScore, index) {
								...
								return (<div className={questionScore.score === 100 ? 'is-correct' : 'is-not-correct'}>
									<p className="pad">{'Question ' + (index + 1) + ' - ' + (questionScore.score === 100 ? 'Correct:' : 'Incorrect:')}</p>
