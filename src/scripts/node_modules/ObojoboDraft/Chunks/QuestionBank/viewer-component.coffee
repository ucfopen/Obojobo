	...
	componentWillMount: ->
		...
		if assessment
			if typeof assessment.current.data[@props.model.get('id')] is 'undefined'
				selectionHistory = AssessmentUtil.getDataForAssessment @props.moduleData.assessmentState, @props.model, 'questions'
				if typeof selectionHistory is 'undefined' or selectionHistory.length is 0
					...
					AssessmentUtil.registerDataForAssessment @props.model, 'questions', questions

				AssessmentUtil.registerDataForCurrentAttempt @props.model, @props.model.get('id'), @chooseChildren()

	chooseChildren: ->
		questions = Array.from AssessmentUtil.getDataForAssessment(@props.moduleData.assessmentState, @props.model, 'questions')
		chosen = []
