...
Question = React.createClass
	statics:
		...
		deleteText: (selection, chunk, deleteForwards) ->
			...
			if not deleteForwards and info.offset is 0 and info.groupIndex is 1
				return

			console.clear()
			console.log 'DT', info.groupIndex, data.textGroup.length - 1

			if deleteForwards and info.offset is info.text.length and info.groupIndex is 0
				return
		...
		splitText: (selection, chunk, shiftKey) ->
			...
			item = data.textGroup.get(info.groupIndex)

			if info.groupIndex is 0
			...
			# 		afterNode.componentContent.textGroup = data.textGroup.split info.groupIndex

			# 	inbetweenNode = Chunk.create()

			# 	data.textGroup.remove info.groupIndex
			...
			data.textGroup.splitText info.groupIndex, info.offset, (data) -> { score:0 }

			selection.setFutureCaret chunk, { offset:0, groupIndex:info.groupIndex + 1}
		...
		selectAll: (selection, chunk) ->
			...
			selection.setFutureStart chunk, { offset:0, groupIndex:info.groupIndex }
			selection.setFutureEnd chunk, { offset:info.text.length, groupIndex:info.groupIndex }

			console.log selection.chunk.futureStart, selection.chunk.futureEnd
		...
		if not @state.active
			...
			React.createElement('div', { contentEditable:false, style:{border:'1px solid green'} },
				...
				React.createElement('div', { style:{border:'1px solid green'}},
					React.createElement('p', null, 'QUESTION:'),
					React.createElement('div', { contentEditable:true, suppressContentEditableWarning:true, style:{border:'1px solid gray'} },
						Text.createElement(data.textGroup.first.text, chunk, 0)
					),
					React.createElement('ul', null,
						data.textGroup.items.slice(1).map (item, i) ->
							React.createElement('li', null,
								React.createElement('div', null,
									...
									React.createElement('span', { contentEditable:true, suppressContentEditableWarning:true, style:{border:'1px solid gray'} }, Text.createElement(item.text, chunk, i+1))
								)
							)
					)
				)
