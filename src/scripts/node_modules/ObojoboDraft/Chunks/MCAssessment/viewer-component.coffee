...
MCAssessment = React.createClass
	...
	getResponseData: ->
		...
		for child in @props.model.children.models
			...
			if QuestionUtil.getResponse(@props.moduleData.questionState, child)?.set
	...
	calculateScore: ->
		...
		switch @props.model.modelState.responseType
			when 'pick-all'
				...
				score = 100
				correct.forEach (id) -> if not responses.has(id) then score = 0
				return score
	...
	onClick: (event) ->
		...
		switch @props.model.modelState.responseType
			when 'pick-all'
				QuestionUtil.recordResponse mcChoiceId, {
					set: !QuestionUtil.getResponse(@props.moduleData.questionState, OboModel.models[mcChoiceId])?.set
				}

			else # pick-one | pick-one-multiple-correct
				for child in @props.model.children.models
					if(child.get('id') isnt mcChoiceId)
