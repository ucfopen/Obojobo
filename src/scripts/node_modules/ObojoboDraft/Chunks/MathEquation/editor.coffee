...
MathEquation = React.createClass
	...
	render: ->
		...
		`<FocusableChunk className="obojobo-draft--chunks--math-equation pad" onKeyDown={this.onAnchorKeyDown} onDoubleClick={this.edit}>
			<div className={( hasError ? ' error' : '' ) + ( isEmpty ? ' empty' : '' )}>
				...
				{ isEditing ?
					...
					:
					<button className="edit-button" onClick={this.edit} style={editButtonStyles}>Edit</button>
				}
			</div>
		</FocusableChunk>`
