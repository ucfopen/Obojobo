...
TableControls = React.createClass
	...
	highlight: (className, action, tdClassName, img) ->
		...
		tds = if tdClassName is null then el.parentElement.getElementsByTagName('td') else el.parentElement.getElementsByClassName(tdClassName)
		for td in tds
	...
	render: ->
		...
		addRowAboveBtn  = `<button
			...
			onClick={addRow.bind(null, cellPos.row)}
		...
		addRowBelowBtn = `<button
			...
			onClick={addRow.bind(null, cellPos.row + 1)}
		...
		addColLeftBtn = `<button
			...
			key='button-add-left'
			onClick={addCol.bind(null, cellPos.col)}
		...
		addColRightBtn = `<button
			...
			onClick={addCol.bind(null, cellPos.col + 1)}
		...
		removeRowBtn = `<button
			...
			onClick={removeRow.bind(null, cellPos.row)}
		...
		removeColBtn = `<button
			...
			onClick={removeCol.bind(null, cellPos.col)}
