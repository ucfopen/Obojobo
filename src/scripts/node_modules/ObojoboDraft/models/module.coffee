	...
	__print: ->
		...
		for chunk in @chunks.models
			if chunk.componentContent?.textGroup? and chunk.componentContent.textGroup.length isnt 0
				for t in chunk.componentContent.textGroup.items
...
Module.createFromDescriptor = (app, descriptor) ->
	...
	for chunkDescriptor in descriptor.chunks
		...
		try
			console.log 'here', descriptor
			chunkDescriptor.draftId = descriptor.id
			console.log 'now', chunkDescriptor
			chunks.push new Chunk(chunkDescriptor)
		catch e
			console.error 'ERROR', e
			console.log ComponentClassMap.getErrorComponentClass()
			# chunks.push Chunk.create(ComponentClassMap.getErrorComponentClass())
