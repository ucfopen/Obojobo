...
class Module extends Backbone.Model
	...
	onChunkAdd: (model, collection, options) ->
		...
		console.log 'after', model.module
	...
	fromDescriptor: (descriptor) ->
		...
		newModule = Module.createFromDescriptor @editor, descriptor
	...
	__print: ->
		...
		for chunk in @chunks.models
			if chunk.componentContent?.textGroup? and chunk.componentContent.textGroup.length isnt 0
				console.log chunk.id + '|' + chunk.get('index') + '|' + chunk.get('type') + ':"' + chunk.componentContent.textGroup.get(0).text.value + '"'
			else
				console.log chunk.id + '|' + chunk.get('index') + '|' + chunk.get('type') + ':<EMPTY>'



Module.createFromDescriptor = (editor, descriptor) ->
	m = new Module editor, descriptor.id
