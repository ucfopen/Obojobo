...
class Chunk extends Backbone.Model
	...
	getComponent: ->
		console.log 'we get compoment for', @get('type'), ComponentClassMap.getClassForType(@get('type'))
	...
	callComponentFn: (fn, sel, content) ->
		...
		if not componentClass.commandHandler?[fn] then return null
		componentClass.commandHandler[fn].apply componentClass.commandHandler, [sel, @].concat(content)
	...
	canRemoveSibling: (selection) ->
		@callComponentFn 'canRemoveSibling', selection
	...
	canMergeWith: (selection, digestedChunk) ->
		@callComponentFn 'canMergeWith', selection, [digestedChunk]
	...
	selectStart: (selection) ->
		@callComponentFn 'selectStart', selection

	selectEnd: (selection) ->
		@callComponentFn 'selectEnd', selection
...
Chunk.create = (typeOrClass = null, content = null) ->
	...
	if not typeOrClass?
		...
		console.log 'cc be', componentClass
		type = typeOrClass
	else
