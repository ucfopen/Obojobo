...
class StyleableText {
	...
	replaceText(from, to, text) {
		if ((text == null) || (text.length === 0)) { return this.deleteText(from, to); }
	...
	toggleStyleText(styleType, from, to, styleData) {
		...
		let styleRange = trimStyleRange(new StyleRange(from, to, styleType, styleData), this.value.length);
	...
	merge(otherText, atIndex) {
		...
		if (atIndex == null) { atIndex = this.value.length; }
	...
	__debug_print() {
		...
		let j = 0;
		return Array.from(this.styleList.styles).map((style) =>
...
StyleableText.getStylesOfElement = function(el) {
	...
	switch (computedStyle.getPropertyValue('font-weight')) {
		case "bold": case "bolder": case "700": case "800": case "900": styles.push({ type:StyleType.BOLD }); break;
	}

	switch (computedStyle.getPropertyValue('text-decoration')) {
		case "line-through": styles.push({ type:StyleType.STRIKETHROUGH }); break;
	}

	switch (computedStyle.getPropertyValue('font-style')) {
		case "italic": styles.push({ type:StyleType.ITALIC }); break;
	}

	switch (computedStyle.getPropertyValue('font-family').toLowerCase()) {
		case "monospace": styles.push({ type:StyleType.MONOSPACE }); break;
	}
	...
	switch (el.tagName.toLowerCase()) {
		...
		case 'a':
			if (el.getAttribute('href') != null) { styles.push({ type:StyleType.LINK, data:{href:el.getAttribute('href')} }); }
			break;
		case 'q':               styles.push({ type:StyleType.QUOTE, data:el.getAttribute('cite') }); break;
		//@TODO:
		// when 'abbr', 'acronym' then styles.push { type:StyleType.COMMENT, data:el.getAttribute('title') }
		case 'sup':             styles.push({ type:StyleType.SUPERSCRIPT, data:1 }); break;
		case 'sub':             styles.push({ type:StyleType.SUPERSCRIPT, data:-1 }); break;
	}
		// @TODO:
		// when 'span'
	...
	switch (node.nodeType) {
		...
		case Node.ELEMENT_NODE:
			...
			for (let style of Array.from(styles)) {
				let styleRange = new StyleRange(state.curText.value.length, Infinity, style.type, style.data);
