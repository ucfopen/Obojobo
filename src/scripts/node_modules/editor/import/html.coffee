...
createOboNodesFromDOMNode = (node, chunkClipboard) ->
	...
	index = node.getAttribute('data-component-index')
	console.log 'FOUND', type, id, index
	if type? and id? and index?
		console.log 'chunkClipboard', chunkClipboard
		console.log '--look', id, chunkClipboard.chunks, chunkClipboard.chunks[id]
		chunk = chunkClipboard.get id

		if chunk?
			console.log ' --FOUND', chunk, chunk.componentContent
			clone = chunk.clone()
			# if clone.componentContent.textGroup.items.length is 0
			# 	# console.clear()
			# 	# debugger;
			# 	console.log '%%%%%%%%%%%%%'
			# 	console.log '%%%%%%%%%%%%%'
			# 	console.log '%%%%%%%%%%%%%'
			# 	console.log '%%%%%%%%%%%%%'
			# 	console.log chunk, chunk.get('id')
			# 	console.log clone
			# 	console.log chunkClipboard.allChunksById
			return [clone]
	...
	p = Chunk.create()
	console.log p.componentContent.textGroup.first
	p.componentContent.textGroup.first.text.insertText 0, node.textContent

	console.log 'rtn', p, p.componentContent.textGroup.first.text
	[p]
...
completeFragment = (oboNodes, fragment, chunkClipboard) ->
	if fragment.childNodes.length > 0
		createdOboNodes = createOboNodesFromDOMNode fragment, chunkClipboard
...
getOboNodesFromHTML = (html, chunkClipboard) ->
	...
	getOboNodesFromHTMLHelper oboNodes, container, fragment, chunkClipboard
...
getOboNodesFromHTMLHelper = (oboNodes, el, curFragment, chunkClipboard) ->
	...
	for node in Array.prototype.slice.call(el.childNodes)
		...
		createdOboNodes = createOboNodesFromDOMNode node, chunkClipboard

		console.log 'createdOboNodes', createdOboNodes
		...
		if createdOboNodes.length > 0
			...
			if node.nodeType is Node.ELEMENT_NODE
				getOboNodesFromHTMLHelper node, clone, null, chunkClipboard
