...
EditorPage = React.createClass
	...
	send: (fn, chunkOrChunks, data) ->
		Util.send fn, chunkOrChunks, @props.selection, data
	...
	onKeyDown: (event) ->
		...
		switch event.keyCode
			...
			when Keyboard.BACKSPACE, Keyboard.DELETE
				...
				if @props.selection.chunk.type is 'caret'
					caretEdge = @send 'getCaretEdge', @props.selection.chunk.start.chunk
					...
					Util.deleteSelection @props.selection
			...
			when Keyboard.TAB
				...
				if @props.selection.chunk.type is 'chunkSpan'
					# ...indent all of them
					@send 'indent', @props.selection.chunk.all, [event.shiftKey]
					@props.updateFn()
					return
				...
				if caretEdge is 'start' or caretEdge is 'startAndEnd'
					# ...indent
					@send 'indent', @props.selection.chunk.start.chunk, [event.shiftKey]
					@props.updateFn()
					return

				# Otherwise, let the chunk figure out what to do
				@send 'onTab', @props.selection.chunk.start.chunk, [event.shiftKey]
				@props.updateFn()

				# Util.deleteSelection @props.selection
				# @send 'insertText', @props.selection.chunk.start.chunk, ["\t"]
				# @props.updateFn()
			...
			when Keyboard.ENTER
				event.preventDefault()
				Util.deleteSelection @props.selection
				@send 'splitText', @props.selection.chunk.start.chunk, [event.shiftKey]
				@props.updateFn()
			...
			when Keyboard.LEFT_ARROW, Keyboard.RIGHT_ARROW, Keyboard.UP_ARROW, Keyboard.DOWN_ARROW
				# @history.add null, @props.selection.getSelectionDescriptor()

				if not @props.styleBrush.isClean
					@props.styleBrush.clean()
					@props.updateFn()

				# @prevSel = @props.selection.getSelectionDescriptor()
		...
		if metaOrCtrlKeyHeld
			switch event.keyCode
				...
				when 66 #b
					...
					Util.activateStyle StyleType.BOLD, @props.selection, @props.styleBrush
				...
				when 73 #i
					...
					Util.activateStyle StyleType.ITALIC, @props.selection, @props.styleBrush
				...
				# when 65 #a
				# 	if @props.selection.chunk.type isnt 'chunkSpan'
				# 		if @send 'selectAll', @props.selection.chunk.start.chunk
				# 			event.preventDefault()
				# 			# @props.updateSelectionFn()
				# 			@props.updateFn()
