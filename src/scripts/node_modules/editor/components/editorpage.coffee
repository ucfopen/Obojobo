...
EditorPage = React.createClass
	...
	getInputActivityData: ->
		...
		chunk: chunk
		savedDOMState: chunk.getDOMStateBeforeInput()
	...
	onInput: (event) ->
		...
		return #@TODO
		console.log '!!!!!!!!!!!!!!!!!!!!!!!!EXPENSIVE INPUT EVENT BEING HANDLED'
	...
	onPaste: (event) ->
		...
		selType = @props.selection.virtual.type
		if selType is 'caret' or selType is 'textSpan'
			console.clear()
			console.log 'gonna paste'
			pasteSuccessful = @props.selection.startChunk.paste text, html
			console.log '-------'
			console.log '-------'
			console.log '-------'
			console.log '-------'
		...
		if text.indexOf('id:') is 0
			...
			if chunk?
				...
				@send 'splitText', @props.selection.startChunk
				splitNode = @props.selection.startChunk.nextSibling()
				splitNode.addChunkBefore chunk
	...
	onMouseUp: (event) ->
		setTimeout @props.updateSelectionFromDOMFn.bind(null, @)
		true
	...
	sendText: (char) ->
		...
		ChunkUtil.deleteSelection @props.selection
		@send 'insertText', @props.selection.startChunk, [char, styleBrush.stylesToApply, styleBrush.stylesToRemove]
		@props.saveAndRenderModuleFn()
		# @props.saveAndRenderModuleFn(char isnt " ")
	...
	componentDidUpdate: ->
		...
		# # if futureDesc
		# # 	console.log 'FUTURE DESC'
		# # 	console.log futureDesc
		...
		if @forcedRerender?
			switch @forcedRerender.activity
				...
				when 'input'
					@forcedRerender.chunk.applyDOMModification @forcedRerender.domModification
	...
	render: ->
		...
		curChunkSel = VirtualSelection.fromDOMSelection @props.module
