...
EditorApp = React.createClass
	...
	onSideMenuStartDrag: ->
		@setState { controlsEnabled:false }

	onSideMenuDrop: (chunk) ->
		@setState { controlsEnabled:true }
	...
	onToolbarCommand: (command, data) ->
		...
		switch command.id
			when 'textType'
				...
				@update()
			when 'bold'
				ChunkUtil.activateStyle StyleType.BOLD, @state.selection, @state.styleBrush
				@update()
			when 'italic'
				ChunkUtil.activateStyle StyleType.ITALIC, @state.selection, @state.styleBrush
				@update()
	...
	replaceSelection: (componentClass, data = null) ->
		...
		# @props.selection.selectFromDescriptor @props.module, @selection.getSelectionDescriptor()
	...
	editChunk: (chunk, controlsEnabled = false, stopEditingListener = ->) ->
	...
	render: ->
		...
		controlsEnabled = @state.controlsEnabled and @state.modalElement is null and not @state.loading
