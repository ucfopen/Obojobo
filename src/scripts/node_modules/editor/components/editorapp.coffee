...
# MutationPainter = require 'ObojoboDraft/debug/mutationpainter'
...
EditorApp = React.createClass
	...
	onSideMenuClick: (position, componentClass) ->
		...
		if position is 'before'
			componentClass.onInsert 'before', @selection.chunk.start.chunk, @selection, null, ((newChunk) ->
			...
			newChunk = componentClass.onInsert 'after', @selection.chunk.end.chunk, @selection, null, ( ->
				@setState { loading:false }
	...
	render: ->
		...
		return `<div className={'editor--components--editor-app ' + (controlsEnabled ? 'controls-enabled' : 'controls-disabled')} onKeyDown={this.onKeyDown}>
