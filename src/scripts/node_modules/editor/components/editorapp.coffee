...
EditorApp = React.createClass
	...
	addPage: (index) ->
		...
		newPage = new Page()
		newPage.chunks.add(newChunk)
		newPage.app = @
		@state.module.pages.add(newPage, { at:index + 1 })

		@selection.setPage(newPage)
		...
		# 	# @updateSelectionFromDOM()
		# 	console.clear()
		# 	console.log(@refs['editorPage' + @state.activePageIndex])
		# 	@refs['editorPage' + @state.activePageIndex].selectStart()
		# ).bind(@), 1000

	onPageMouseDown: (page, pageIndex) ->
		if pageIndex isnt @state.activePageIndex
			# @needSelectionUpdate = true
			# console.log('on page click', page.cid, pageIndex)
	...
	deletePage: (page) ->
		...
		if @state.module.pages.models.length is 1
			alert '@TODO'
		else if pageIndex is 0
			selPage = @state.module.pages.at(pageIndex + 1)
			@selection.setPage(selPage)
			selPage.chunks.at(0).selectStart()
			...
			selPage = @state.module.pages.at(pageIndex - 1)
			@selection.setPage(selPage)
			selPage.chunks.at(selPage.chunks.models.length - 1).selectEnd()
			...
			`<div key={index} ref={'pageContainer' + index} className={'page-container' + (isActive ? ' active' : '')}>
				{
					index === 0
					?
					<button className="add-page-btn" onClick={addPage.bind(null, -1)}>+ Add Page</button>
					:
					null
				}
				<EditorPage
				...
				<button className="add-page-btn" onClick={addPage.bind(null, index)}>+ Add Page</button>
			</div>`
		).bind(@))
