...
EditorApp = React.createClass
	...
	_onAppendMenuClick: (componentClass) ->
		...
		newChunk = onInsert componentClass, 'after', self.state.module.chunks.at(self.state.module.chunks.length - 1), self.selection, ->
			self.setState { loading:false }
			self.saveAndRenderModule()
			...
			self.scrollToBottomAfterUpdate = true
			# ).bind(@), 1000
	...
	_onMouseUp: (event) ->
		if not @editingChunk? and not DOMSelection.includes(ReactDOM.findDOMNode(this.refs.editorPage))
			@renderModule()
	...
	componentDidUpdate: ->
		...
		if @scrollToBottomAfterUpdate
			delete @scrollToBottomAfterUpdate
