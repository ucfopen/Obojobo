...
class TextGroupCommandHandler extends BaseCommandHandler
	...
	deleteText: (selection, chunk, deleteForwards) ->
		...
		if not deleteForwards and s.isTextStart and not s.isFirstGroup
			# ...merge that text with the previous one
			selection.setFutureCaret chunk, { offset: data.textGroup.get(s.groupIndex - 1).text.length, groupIndex: s.groupIndex - 1}
			data.textGroup.merge s.groupIndex - 1, mergeDataFn
			return true

		# If using delete key on the end of a text that's not the last text...
		if deleteForwards and s.isTextEnd and not s.isLastGroup and data.textGroup.length > 1
			# ...merge that text with the next one
			selection.setFutureCaret chunk, { offset: s.offset, groupIndex: s.groupIndex }
			data.textGroup.merge s.groupIndex, mergeDataFn
			return true
	...
	split: (selection, chunk) ->
		...
		allTextSelected = sel.start.isFirstGroup and sel.end.isLastGroup
		...
		if middle.componentContent.textGroup.length is 0
	...
	restoreSelection: (selection, chunk, type, savedSelData) ->
		FocusableCommandHandler.prototype.restoreSelection selection, chunk, type, savedSelData
	...
	getTextMenuCommands: (selection, chunk) ->
		[
			...
			},
			{
				label: 'TEST'
				fn: (selection, chunk) ->
					chunk.styleSelection selection, 'u', { someData:'goesHere' }
			}
		]
