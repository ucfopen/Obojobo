...
class TextGroupCommandHandler extends BaseCommandHandler
	...
	getDOMSelectionStart: (selection, chunk) ->
		node = Text.getTextNode chunk, selection.virtual.start.data.groupIndex
		return null if not node?

		Text.getDomPosition selection.virtual.start.data.offset, node

	getDOMSelectionEnd: (selection, chunk) ->
		node = Text.getTextNode chunk, selection.virtual.end.data.groupIndex
		return null if not node?

		Text.getDomPosition selection.virtual.end.data.offset, node
